...
class ROI:
    ...
    def motion_notify_callback(self, event):
        ...
        if event.inaxes:
            ...
            if event.button == None and self.line != None:  # Move line around
                ...
                line = pyl.Line2D([self.previous_point[0], x],
                                  [self.previous_point[1], y])
        ...
        if event.inaxes:
            ...
            if event.button == 1:  # If you press the left button
                if self.line == None:  # if there is no line, create a line
                    self.line = pyl.Line2D([x, x], [y, y], marker='.')
                ...
                # add a segment
                    ...
                    self.line = pyl.Line2D([self.previous_point[0], x],
    ...
    def get_indices(self):
        ...
        if coo is None:
            return None
        # temp = self.fig.axes[0].images[0].get_array()
        xend = self.fig.axes[0].dataLim.intervalx[1]
        yend = self.fig.axes[0].dataLim.intervaly[1]
        xlist = [i / self.im[1] * xend for i in range(int(self.im[1]))]
        ylist = [i / self.im[0] * yend for i in range(int(self.im[0]))]
        ...
        x, y = coo  # was originally y,x = coo
        x = [find_nearest_index(xlist, i) for i in x]
        y = [find_nearest_index(ylist, i) for i in y]
        return polygon(y, x, self.im)  # the polygon function takes (rows, columns) as its argument
...
class ROIcircle(ROI):
    ...
    def __setstate__(self, d):
        self.im = d['im']
        self.circ = pyl.Circle(*d['circle'], facecolor='none', edgecolor='b')
        ...
        if event.inaxes:
            ...
            if event.button == 1:  # If you press the left button
                if self.circ == None:  # if there is no line, create a line
                    self.circ = pyl.Circle((x, y), 0.5, facecolor='none', edgecolor='b')
...
def new_ROI(im=None, shape='polygon'):
    ...
    axes."""
    pyl.ion()
    im = pyl.gci() if im is None else im
